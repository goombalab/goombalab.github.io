...
They can be viewed as a modern version of an recurrent neural network (RNN) with a few key characteristics.
While a lot of technical work was involved in getting this family of models to work, I'll start by trying to abstract away what I view as the main high-level ingredients to help these models match the performance of Transformers on language modeling.
...
Mamba addressed this with careful parameterization of the recurrence and utilizing the classic parallel scan algorithm CITE.

Many other algorithmic innovations have appeared, all with a few shared characteristics:
- They aim to be parallelizable and practically efficient on accelerators like GPUs and TPUs -- preferably leveraging matrix multiplications (matmuls) as the workhorse.
- They have to control memory management carefully. In particular, any model that uses state expansion can't actually materialize the state in main memory! While Mamba brute-forced the problem using clever awareness of the GPU memory hierarchy, most alternatives find ways of rewriting the equations entirely to use different computation paths that don't need to compute the state explicitly during a parallel training pass.
- The model has to be linear in $x_t$, leading some to call this whole family of models *linear recurrent models*.
...
3. Parallel scans were utilized in earlier linear RNNs like S5 <d-cite key="smith2023s5"></d-cite> and LRU <d-cite key="orvieto2023resurrecting"></d-cite>. Linear attention variants also used parallelizable training algorithms leveraging matmuls.

What Mamba did was show that *combining all of these together* was the key to approaching Transformer performance on language modeling.


### Modern recurrent models
Since then, there has been a flurry of activity on continuing to understand and improve recurrent models.
Many of them come from different motivations with different nomenclatures and terminologies.
- Some models such as RWKV <d-cite key="peng2023rwkv"></d-cite><d-cite key="peng2024eagle"></d-cite><d-cite key="peng2025rwkv"></d-cite>, xLSTM <d-cite key="katharopoulos2020transformers"></d-cite>, and Griffin CITE come from an **RNN-centric** point of view and call Ingredient 1 *matrix-valued states* and Ingredient 2 *gating*.
- **Linear attention** <d-cite key="katharopoulos2020transformers"></d-cite> was the first to combine Ingredients 1 and 3; later variants such as GLA<d-cite key="yang2024gated"></d-cite> and Gated DeltaNet<d-cite key="yang2025gated"></d-cite> incorporate various forms of selectivity (data-dependent recurrence) and use attention-based terminology such as using $(K, Q, V)$ instead of $(B, C, X)$.
- Recently, many of these models have been cast into a framework of **test-time training/regression**<d-cite key="liu2024longhorn"></d-cite><d-cite key="sun2024learning"></d-cite><d-cite key="wang2025test"></d-cite><d-cite key="von2025mesanet"></d-cite>, which views the recurrent update as taking gradient steps on some objective that optimizes for remembering the context. The state is viewed as an *associative memory* and parallelization happens through a notion of *minibatch gradient descent*.


A key commonality is that almost all of these models can be cast into the same SSM equation \eqref{eq:ssm}, with the main axes of variations being in the structure of $A_t$ and corresponding efficient training algorithms.
...
Despite the accelerating amount of research into this direction and steady stream of new models, however, I think that all of these models are still quite similar to each other and have roughly similar empirical performance, for the most part.
In particular, *all of these models are much more similar to each other than they are to quadratic attention*.
So in the rest of this blog post, we're going to try to get a grasp on the high-level trade-offs between SSMs and Transformers.
...
What does that mean?
In some sense, every autoregressive model -- one that generates data sequentially left-to-right -- is a "state space model" that holds some state in memory and evolves it on every time step (e.g. in between every generated word for a language model).

### Autoregressive states of sequence models

Self-attention, the core component of Transformers, is often defined through a specific operation involving computing the pairwise interactions between every element of the sequence. CITE
...
- On the other hand, a state space model has always summarized its context $x_1, \cdots, x_t$ into the hidden state $h_t$ (equation \eqref{eq:ssm}), which always has a constant size. This fixed-size state is the only [means] by which the model can interact with data: it streams data in, compresses it into its state, and uses that to interact with the environment or produce new outputs.
...
This is, of course, more formally known as the *KV cache*. But the point of this description is that I think that rather than defining the KV cache as a consequence of attention, perhaps in an alternative universe (causal) self-attention could have been derived from first principles as the canonical model that stores a cache ("KV" or not) of its context. So in the rest of this post, I'll will try to call it a "context cache" or "token cache" instead to abstract out the main principle instead of implementation detail.
...
For example, SSMs can't memorize a phonebook in one pass and then recite it back, or recall an arbitary person's phone number from memory CITE. But then of course, neither can humans - we're hopelessly bad at exact memorization and retrieval (but that doesn't seem to hinder intelligence from arising!).
...
The theme here is that sometimes limitations of methods are not so black-and-white. They can depend on the way in which models interact with data and more generally on higher system-level changes. But we're not going to get into these nuances for the purposes of this post.
{% enddetails %}
...
Both Transformers and SSMs have been touted as having "long-context abilities".

The reason is because they have very different *types* of memory.
Going back to the analogy, I wouldn't say that if comparing, say, my own memory vs. my research journal, that there is a clear winner. They're both just different: my journal lets me refer back to specific details I may have forgotten, but my brain remembers a much longer history of fuzzy context.
...
And what's even more intriguing is that the optimal ratio of these layers, as independently verified by dozens of research groups by now ([Jamba](https://arxiv.org/abs/2403.19887), [Zamba](https://arxiv.org/abs/2405.16712), [Samba](https://arxiv.org/abs/2406.07522), and many more that followed after)<d-cite key="lieber2024jamba"></d-cite><d-cite key="glorioso2024zamba"></d-cite><d-cite key="ren2025samba"></d-cite>, is somewhere between a roughly 3:1 to 10:1 ratio of SSM:attention layers.<d-footnote>Note that this isn't factoring in computation cost (which is usually what's highlighted when comparing Transformers vs SSMs) - we're just talking about raw modeling ability. Put another way, taking a pure Transformer model and replacing some of the layers with SSM layers would both improve efficiency *and* performance.</d-footnote>
This might track the coarse analogy if one believed that human intelligence is mostly in the brain and augmented by a lightweight access to external databases!
...
I actually believe that pound-for-pound (i.e. FLOP-for-FLOP), SSMs are better than Transformers at modeling language, in this sense.
...
There's a perception of Transformers being the ultimate architecture that can learn anything from raw data, the more the better, with more compute being the only bottleneck.
...
Tokenization is a notorious step of all language modeling pipelines, where raw textual data is processed into contiguous chunks, essentially encoding them into coarser features than the raw character-level data.
It has a number of failure modes such as the [SolidGoldMagikarp](https://www.lesswrong.com/posts/aPeJE8bSo6rAFoLqg/solidgoldmagikarp-plus-prompt-generation) edge case and the "How many R‘s are there in the word 'strawberry'?" test.
...
From polling a lot of opinions, almost everyone agrees that tokenizers are clunky and ugly, but a necessary evil. Practically speaking, they sub-sample the sequence by a factor of around $5\times$ which dramatically improves the efficiency of the core language model.
...
The first thing to note is that, perhaps not surprisingly, a FLOP-matched SSM performs *much* better than a FLOP-matched Transformer. Most people expect this because byte sequences are much longer than BPE-token sequences, and the quadratic complexity of Transformers kick in.

But as I said earlier, the weakness of Transformers is not (just) about efficiency, but about (certain aspects of) modeling power.
And what's notable about this plot is that **when matching for *data* instead of compute, allowing the Transformer to use many more FLOPs, the SSM still outperforms it substantially**!
...
{% include figure.liquid loading="eager" path="assets/img/2025-07-09-tradeoffs/dna.png" %}
...
For example, when reading, we pay attention to units at the level of words (or subwords like prefixes/suffixes), which have *meaning*.
...
On the other hand, when the data is generally not meaningful (in the sense of requiring a model to pay attention to individual units), such as character-level language or DNA, Transformers don't work well, and other models like SSMs hold a clear edge.
...
Despite being motivated by and focusing on language modeling in the paper, the majority of its applications were actually in other modalities!<d-footnote>I don't work in computer vision, and part of me is unsure how much of Mamba's popularity there is just trend following 😜 but I've been told at least that SSMs are pretty effective!</d-footnote>
...
2. [An Analysis of Tokenization: Transformers under Markov Data](https://openreview.net/forum?id=wm9JZq7RCe) showed that for certain data distributions, applying tokenization qualitatively changes what Transformers can learn. Intuitively, commonly used tokenizers are somewhat based in information-theoretic heuristics, and play a particular role in smoothing out the non-uniform information rate of raw data into a form that's more easily processed by a Transformer.<d-cite key="rajaraman2024analysis"></d-cite>
{% enddetails %}
...
Another thought experiment that's intrigued me is what happens in the presence of noise. LLM data famously requires immense amounts of processing, filtering, and cleaning, but real-world data (and other modalities) aren't like that.
...
I think this shows another clear failure mode of standard attention: the compute shouldn't be scaling as $k^2$, and the memory certainly shouldn't scale in $k$ either.
...
And I'll informally propose this one as a goal for the future of architecture design -- maybe someone will take me up on it and formalize it in a paper someday.
...
When [S4](https://arxiv.org/abs/2111.00396) was published, I (and many others) were excited about its potential on various modalities, and it spawned a wave of follow-up work on [pure convolutional language models](https://arxiv.org/abs/2302.10866).<d-cite key="gu2022efficiently"></d-cite><d-cite key="poli2023hyena"></d-cite>
...
Clearly, even simple mis-spacings would drastically change what features a convolution could pick up on -- in the above example, the convolution could not possibly output the same feature on both of those input sequences, in contrast to sequence mixing layers like attention or selective SSMs.
...
I think this framing gives us better intuition of how these variants might behave.
...
Incidentally, this is another more subtle reason why I somewhat prefer using "state space model" or "recurrent model" over "linear attention" as a descriptive term.
...
To end, let's talk about one of the major drivers of the current wave of progress in AI: **scaling laws**, or the phenomenon that spending more compute on models consistently leads to more capabilities.

These laws are always plotted with FLOPs on the x-axis and some measure of performance on the y-axis, with the idea that the slope of this line measures "the rate at which FLOPs are converted into capabilities".
Indeed, I've heard the viewpoint that Transformers are simply a vehicle that optimally performs this conversion.
...
Don't get me wrong: despite being known as a leader of the Transformer alternative direction, I think Transformers are amazing and attention is truly a fundamental modeling primitive.
...
Part of my reason for writing this post was to broadcast this talk to a wider audience, and so I don't have to go around giving talks anymore :)
