...
I was particularly interested in the idea of improving long-range dependencies through hierarchies.
...
[Ricardo Buitrago](https://www.linkedin.com/in/ricardobuitrago/) explored it a little bit but didn't get too far as he started working on another project on [length generalization in recurrent models](https://goombalab.github.io/blog/2025/improving-length-generalization/) <d-cite key="buitrago2025understanding"></d-cite> (I'm also very fond of this result!).<d-footnote>Actually, Ricardo did have a couple quite interesting results that didn't go into the H-Net paper but that we might figure out a way to. 
...
In their paper, this fully end-to-end version seemed promising but not ideal; it had stability issues and underperformed their alternative variants that relied on more heuristics and supervision.
...
Since I expect to get a lot of questions about how H-Net compares against BLT, I'll be really explicit here. Happy to be corrected by the BLT authors (or anyone else) if I say anything incorrect!

**tl;dr**
- BLT is not actually end-to-end
- H-Net should both be much easier to use and perform better than BLT
...
The main advantage of H-Net is that it's *actually end-to-end*, learning the strategy together with the model.

Additionally, the entropy-based approach of BLT has some intrinsic weaknesses: 
- It requires significant extra work (to train an auxiliary conditional prediction model).
- It can't be nested into deeper hierarchies, which was one of the core motivations of H-Net and we showed significantly improved performance. In every experiment we compared, we found H-Net (2-stage) > H-Net (1-stage).
...
Instead of an entropy heuristic, SpaceByte simply chunks on certain delimiters in the text.
...
Note that the advantage of BLT over SpaceByte is that the entropy strategy is *more flexible*: it can be applied to languages without delimiter-based segmentation strategies, or potentially even non-discrete sequences of data. 
However, it's still strictly less flexible than the dynamic chunking (DC) mechanism of H-Net, so I don't think there are any advantages.

<br>
#### Architecture
The architectural components of the models are generally orthogonal to the chunking strategy.
BLT introduced a number of unusual components, such as a new type of chunk-aware cross-attention layer,
and large tables of "hash n-gram embeddings".
We think both of these are unnecessarily complicated; H-Net works just fine without these additional techniques, although I believe that they could also be incorporated into H-Net if desired.

<br>
So overall, we felt that the full BLT model was an overly complicated baseline to set up and compare against, and (as explained in the paper) we used SpaceByte as a representative example of BLT-like models.
But I still like the idea of BLT and I'm glad they published it!
