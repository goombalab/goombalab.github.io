...
I've been fortunate enough to be able to work with my student [Sukjun](https://sukjunhwang.github.io/) who made all the technical breakthroughs behind this model, solving (or at least making serious progress towards) what I consider to be a very difficult but foundational problem for deep learning.
...
At some point early in my PhD, when I wasn't doing much, I took an online class for fun on [Learning How to Learn](https://www.coursera.org/learn/learning-how-to-learn).
...
Back then, I thought that *chunking* was another such fundamental primitive.
...
The first reason is, as I appealed to in my [[previous blog]({% post_url 2025-07-08-tradeoffs %}#should-we-get-rid-of-tokenization)],
...
to solve tokenization would be through creating a *differentiable chunking module*, which would be so much more powerful.
...
### Stuck.
At this point, tokenization and chunking was one of my personal "holy grail" problems.
...
At the time, I actually basically didn't know at all how MoE worked, so I left Sukjun to his own devices for a long time,
...
[TODO: add a whiteboard picture from Sukjun or an early PowerPoint diagram he drew]
...
Their motivation for how to overcome the discrete-choice problem was delegating to stochastic reparametrization techniques, namely the Gumbel-softmax trick CITE.
...
this conditional distribution would have very low entropy and so the conditional entropy heuristic would create a new chunk.
But in the event that the actual token that appears is a different one $w' \neq w$,
...
I haven't been able to run experiments personally in a while, and it was like watching an artist to see Sukjun improve the architecture steadily over time with multiple creative and essential ideas.
...
[//]: # (If it isn't clear, I didn't do any of this; Sukjun is an amazing empiricist and has by far the best intuition for everything related to H-Nets.)
...
<span style="color:blue">AG: Hmm, wrote this in a stream of thought but feels like it's excessive information?</span>
...
I was more interested in the litmus test as a thought experiment to guide architecture design, not necessarily as an actual synthetic.
I did hypothesize though that something that could theoretically solve the litmus test might be better at working with less clean data,
...
In the previous post, I asked the question: [[Is compression a bug, or a feature?]({% post_url 2025-07-08-tradeoffs %}#state-space-models)]
...
since (as mentioned above) they have similar performance on standard tokenized language modeling.
...
{% include figure.liquid loading="eager" path="assets/img/2025-07-11-hnet/abl_enc_dec_combination_BPE.png" caption="A 1-stage H-Net trained on top of standard BPE-tokenized inputs, with T2M2-M2T2 for example denoting 2 Transformer layers and 2 Mamba layers in the encoder (and the reverse in the decoder). Note that Transformer layers are 2x the size of Mamba layers here." %}
...
I also actually believe that **H-Nets will warp the scaling law exponent**.
However, these aren't formal scaling laws.
...
At no point did it seem worth completely freezing the architecture to run out expensive formal scaling laws, when I think we were able to get an intuitive understanding of their scaling behavior from our proxy protocol;
...
{% include figure.liquid loading="eager" path="assets/img/2025-07-11-hnet/bpb_curve_xl_hnet_white.png" width="50%" caption="On a similar note, I realized we should have just ran this out for much more data to show the gap widening 🤦🏻 <br> We didn't because we only ran things out enough to follow our protocols and provide signal to improving the model." %}

We have a bunch of ideas for where to go from here;
I'll touch on some of these, and also return to scaling laws, in the [next post](2025-07-11).
